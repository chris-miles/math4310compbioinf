...
::: {.biology}
When comparing genes, shared ancestry is one explanation for sequence similarity. Another is that the sequences happen to use the same common letters. Protein comparisons face the same issue, with twenty letters and substitutions that can preserve or change the chemical properties of a residue.
:::
...
The $p_{ab}$ values describe pairs in corresponding positions of related sequences. They are joint probabilities, not probabilities of the second letter conditional on the first: the entire table sums to one. Later, a Markov transition table will instead have rows that sum to one.
